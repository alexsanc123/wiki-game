...
let get_first_item_of_all_unordered_lists contents : string list =
  let open Soup in
  let unordered_nodes = parse contents $$ "ul" in
  let unordered_lists = to_list unordered_nodes in
  let unordered_element_nodes =
    List.filter_map unordered_lists ~f:(fun ul -> ul $? "li")
  in
  List.map unordered_element_nodes ~f:(fun li ->
    texts li |> String.concat ~sep:"" |> String.strip)
;;
...
let get_first_item_of_second_unordered_list contents : string =
  ...
  let unordered_nodes = parse contents $$ "ul" in
  let unordered_lists = to_list unordered_nodes in
  let second_ul_list_node = [ List.nth_exn unordered_lists 1 ] in
  let unordered_element_nodes =
    List.filter_map second_ul_list_node ~f:(fun ul -> ul $? "li")
  in
  List.hd_exn
    (List.map unordered_element_nodes ~f:(fun li ->
       texts li |> String.concat ~sep:"" |> String.strip))
